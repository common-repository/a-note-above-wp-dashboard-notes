...
class WPNAPI extends WP_REST_Controller {
    ...
    public function register_routes() {
        ...
        register_rest_route(
            ...
            array(
                array(
                    'methods'             => \WP_REST_Server::EDITABLE,
                    'callback'            => array( $this, 'tag_search' ),
                    'permission_callback' => array( $this, 'get_items_permissions_check' )
                )
            )
        );
    ...
    private function format_notes( $notes ) {
      ...
      foreach ($notes as $key => $note) {
        ...
        $formatted_note = (object) array(
          ...
          'tags'              => $this->process_tags($note->meta),
          'tempAllVisibility' =>  '',
          'tempContent'       =>  wp_kses($note->content, 'post'),
          'tempRole'          =>  '',
          'tempRoleVisibility'  =>  '',
          'tempSingularVisibility'  =>  '',
          'tempTags'  =>  array(),
          'tempTitle' =>  wp_kses($note->title, 'strip'),
          'visibilityView'  =>  ''
        );
    ...
    private function sanitize_note($note) {
      ...
      $sanitized_note = (object) array(
        ...
        'tags'              =>  $this->sanitize_tags($note->tags) ?? [],
      );
    ...
    private function sanitize_tags($tags) {
      ...
      if (!empty($tags)) {
        ...
        $comma_separated_tags = implode(",", $sanitized_tags);
      }

      return $sanitized_tags;
    }

    private function process_tags($meta_string) {
      $processed_tags = array();
      $meta = json_decode( $meta_string );

      if (is_object($meta)) {

        $raw_tags = $meta->tags;

          foreach ($raw_tags as $key => $tag) {
            $sanitized_tag = sanitize_text_field( $tag );
            array_push( $processed_tags, $sanitized_tag );
          }
    ...
    // Search Tags and return notes with corresponding tags
    public function tag_search( $request )  {

      $body = (array) $request->get_body();

      $query = json_decode($body[0]);

      $search_results = $this->search_tags($query);

      $formatted_notes = $this->format_notes($search_results);

      $return_parcel = (object) array(
        'search_results'    =>  $formatted_notes,
        'status'            =>  true
      );
    ...
    private function search_tags($dirty_query) {
      global $wpdb;

      $query = $this->sanitize_query($dirty_query);
      $table = $wpdb->prefix . '_anoteabove';
      $sql = $wpdb->prepare( "SELECT * FROM $table WHERE meta LIKE %s; ORDER BY id DESC",
          '%' . $wpdb->esc_like($query) . '%'
      );
    ...
    private function sanitize_query($dirty_query) {

      $clean_query = sanitize_text_field( $dirty_query->query );
